...
const Notifies = mongoose.model("Notifies");
...
notifyRouter
  .get("/view", async (req, res, next) => {
    try {
      ...
      let notifyData = await Notifies.find()
        .limit(limit)
        .skip(page)
        .sort(query.orderby)
        .select(query.fields);
      let rowCount = await Notifies.countDocuments();
      buildMessage = {
        ...resultCode(20000),
  ...
  .get("/:targetId", async (req, res, next) => {
    try {
      ...
      let notifyData = await Notifies.find({ targetId: params.targetId });
  ...
  .post("/", async (req, res, next) => {
    ...
    try {
      let notifyData = await Notifies.create(body);
      buildMessage = {
        ...resultCode(20000),
